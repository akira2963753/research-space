...
def plot_selected_histograms(
    ...
    """Plot three fixed Transformer layers for every BFP format."""
...
def main() -> None:
    ...
    results = load_results(profile_dir, formats, args.group_size)
    coverages = compute_coverages(results)
    print_summary(results, coverages)
    # W90/W95/W99 figures are retained as callable exploratory helpers but are
    # no longer part of the default output set; the research now uses full span.
    plot_full_span_heatmap(results, output_dir)
    plot_selected_histograms(results, output_dir)
