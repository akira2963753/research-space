    ...
    function automatic [`FPACC_W-1:0] NORM;
        ...
        begin
            ...
            if (mag != 0) begin
                ...
                // Keep every operand signed. Otherwise unsigned lz makes the
                // whole expression unsigned and zero-extends a negative blk_exp.
                out_exp = $signed(blk_exp)
                        + (`FPACC_EXP_BIAS - `BFP_EXP_BIAS)
                        + $signed((`BFP_MAG_W - 1) - lz); // rebias + leading-one index
